...
queuedata_t getNext(queue_t *queue)
{
    ...
    queuedata_t aux_event = queue->buffer[queue->head];                   // Guardo el dato en un auxiliar para correr la cabeza de la cola
    queue->head = (queue->head == BUFFER_SIZE - 1) ? 0 : queue->head + 1; // Corro la cabeza para apuntar al siguiente dato
...
bool put(queue_t *queue, queuedata_t data)
{
    ...
    if ((queue->tail == (queue->head - 1)) || (queue->head == 0 && queue->tail == (BUFFER_SIZE - 1))) // Si la cola esta un lugar antes que la cabeza significa que esta llena la queue
    {
        queue->isFull = true;
    }
